...
final class Router implements RouterInterface
{
    ...
    public protected(set) array $routes = [] {
        set {
            $routes = $value;
            usort($routes, function (RouteInterface $a, RouteInterface $b) {
                if ($a->path === $b->path) {
                    return 0;
                }
                $len = max(strlen($a->path), strlen($b->path));
                for ($i = 0; $i < $len; $i++) {
                    if (isset($a->path[$i]) === false) {
                        return 1;
                    }
                    if (isset($b->path[$i]) === false) {
                        return -1;
                    }
                    ...
                    return strcasecmp($a->path[$i], $b->path[$i]);
                }
